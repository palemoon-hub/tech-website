<template>
  <div class="flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
    ...
    <main class="flex-1 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col min-w-0">
      ...
      <!-- 6. Color Converter -->
      <div v-if="currentTool === 'color'" class="h-full flex flex-col gap-6">
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h3 class="font-medium text-gray-900 mb-4">{{ $t('imageTools.color.title') }}</h3>
          <div class="space-y-6">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('imageTools.color.select') }}</label>
            ...
            <div class="p-4 bg-gray-50 rounded-lg border border-gray-100">
              <div class="text-xs text-gray-500 mb-2">{{ $t('imageTools.color.preview') }}</div>
              <div 
...
import { Image, FileType, Upload, Download, CheckCircle, Minimize2, Edit3, RotateCcw, RotateCw, MoveHorizontal, MoveVertical, RefreshCcw, Sun, Moon, Stamp, Palette, Pipette } from 'lucide-vue-next'
...
const tools = computed(() => [
  ...
  { id: 'color', name: t('imageTools.nav.color'), icon: Palette },
])
...
const pickColor = async () => {
  if (!('EyeDropper' in window)) {
    pickerError.value = t('imageTools.color.notSupported')
    return
  }
